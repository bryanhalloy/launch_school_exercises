require 'pry'

class Length
  include Comparable #???????
  attr_reader :value, :unit

  def initialize(value, unit)
    @value = value
    @unit = unit
  end

  def as_kilometers
    convert_to(:km, { km: 1, mi: 0.6213711, nmi: 0.539957 })
  end

  def as_miles
    convert_to(:mi, { km: 1.609344, mi: 1, nmi: 0.8689762419 })
  end

  def as_nautical_miles
    convert_to(:nmi, { km: 1.8519993, mi: 1.15078, nmi: 1 })
  end

  def <=>(other)
    converted_other = case unit
                      when :km  then other.as_kilometers
                      when :mi  then other.as_miles
                      when :nmi then other.as_nautical_miles
                      end
    self.value <=> converted_other.value
  end

  
  
  # def ==(other)
  #   case unit
  #   when :km  then value == other.as_kilometers.value
  #   when :mi  then value == other.as_miles.value
  #   when :nmi then value == other.as_nautical_miles.value
  #   end
  # end

  # def <(other)
  #   case unit
  #   when :km  then value < other.as_kilometers.value
  #   when :mi  then value < other.as_miles.value
  #   when :nmi then value < other.as_nautical_miles.value
  #   end
  # end

  # def <=(other)
  #   self < other || self == other
  # end

  # def >(other)
  #   !(self <= other)
  # end

  # def >=(other)
  #   self > other || self == other
  # end

  def to_s
    "#{value} #{unit}"
  end
  

  private

  def convert_to(target_unit, conversion_factors)
    Length.new((value / conversion_factors[unit]).round(4), target_unit)
  end
end

# Example



puts [Length.new(1, :mi), Length.new(1, :nmi), Length.new(1, :km)].sort
# => comparison of Length with Length failed (ArgumentError)
# expected output:
# 1 km
# 1 mi
# 1 nmi



=begin
I can't pinpoint exactly what is triggering the ArgumentError. But, I can see that the class definition is trying to re-build many of the functions in the comparable module. To better write this code and avoic bugs introduced by this module near-re-write, we can just define a custome <=> method for the Lengths class, and include the comparable module. Our <=> method determines the equalivalent values for the other length, and then uses the numerical <=> and comparable methods to compare them. This resolves the error. 


=end

